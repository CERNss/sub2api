...
| 10 | `keepalive-raw-chat-completions-stream`  | 🟢 active   | CC 直转流补下游保活 + 上游空闲上限，长思考不再被前置代理判 504；v0.2.0 起与上游截断检测（#6299）在同一 `finalize` 闭包共存 | 0 | 4 |
| 11 | `add-platform-opencode-templates`        | 🟢 active   | OpenCode tab 支持 openai/grok/zhipu 每平台模版段；内置生成器补 GLM 分支与每平台 model pin；CCS 补 zhipu；GLM 组 Claude Code 模板全槽位钉 glm-5.3 + 1M 窗口，CCS 导入 env 同源 | 0 | 5 |
...
| 路径 | 改动要点 |
|------|---------|
| `backend/internal/service/openai_gateway_grok.go` | 【上游已收编】`normalizeGrokReasoningEffortValue` 的模型参数（3 个调用点）与 xhigh 白名单。【fork 剩余】alias switch 的 `case "xhigh", "extrahigh", "max", "ultra":` 合并写法——上游拆成两支、把 `max`/`ultra` 恒拍平成 `high`，fork 保持四个别名共用白名单判定。～2026-09-05 v0.2.0：上游把白名单函数导出为 `GrokSupportsXHighReasoningEffort`（`handler/gateway_handler.go`、`openai_codex_models_service.go` 复用它给 Codex 目录声明 xhigh 档），fork 合并 case 改调导出名，rebase 时这一行必冲突；语义差不变，上游本轮仍无 `max`/`ultra` 用例，退场条件未满足 |
...
| 路径 | 改动要点 |
|------|---------|
| `backend/internal/service/openai_gateway_chat_completions_raw.go` | 逐行同步循环重构为「读协程 + `select`」（与姊妹函数 `handleChatStreamingResponse` 同构）；新增 `keepaliveInterval`（`gateway.stream_keepalive_interval`，空闲写 `:\n\n`）与 `streamInterval`（Grok 走 `resolveGrokStreamIdleTimeout`，其余走 `gateway.stream_data_interval_timeout`）；`keepaliveWritten` 驱动静默拒答 failover 的 `SafeToFailoverAfterWrite`；`midFrame` 让保活只发生在 SSE 帧边界；`newStreamHeaderWriter` 的用法换成本地两段式提交（`commitStableSSEHeaders` 只提交稳定 SSE 头，`writeStreamHeaders` 才透传上游响应头）；两个定时器都关闭时保留原同步快路径。～2026-09-05 v0.2.0：上游 PR #6299（`4e9c1d7c8`）给同函数加了**上游截断检测**（`openAIRawStreamTerminalState` 记 [DONE]/usage/finish_reason 三种终止信号；未写语义字节时回 `newOpenAIRawStreamTruncatedFailoverError` 换号、已写出后回 `newOpenAIUpstreamStreamReadError` 让 handler 补 SSE error 帧并计入 SLA），与 fork 的读协程结构真冲突，已并入 `finalize` 闭包：`processLine` 里 `terminal.ObserveDataLine(trimmedPayload)`，截断判定放在读错误日志之后、静默拒答之前；**截断 failover 错误按静默拒答同口径带 `SafeToFailoverAfterWrite = keepaliveWritten`**（仅写过保活注释仍可换号），这一行是 fork 独有、上游没有。上游的 `resultWithUsage` 闭包与 fork `buildResult` 同体，保留 fork 名。上游新增的 10 个截断用例（`_TruncatedStreamAfterOutputFailsRequest` 等）都走同步快路径，零改动通过 |
...
| `frontend/src/composables/useModelWhitelist.ts` | `zhipuModels` 跟随 glm-5 阵容，使本 change 在 catalog 与模版里推荐的 GLM 模型可白名单化 |
| `frontend/src/i18n/locales/en/dashboard.ts` | `keys.useKeyModal.zhipu.claudeDescription` / `claudeNote`（GLM 组 Claude Code tab 文案；上游本轮也在同一位置加 deepseek/composite/routedCodex 段，rebase 必冲突，两边都保留）|
| `frontend/src/i18n/locales/zh/dashboard.ts` | 同上中文 |
...
- `frontend/src/views/user/KeysView.vue` → 也属于 #6（`executeCcsImport` 把 `encodeCcSwitchInlineConfig(config.env)` 作为 deeplink `config` 默认值）

#### GLM 组 Claude Code 模板（2026-09-02 `a419dbb81`，2026-09-05 补登记）
`UseKeyModal.vue` 把 Grok Claude tab 的内置生成器抽成 `generatePinnedClaudeFiles(baseUrl, apiKey, model, extraEnv)`，zhipu 组的 Claude tab 走 `generateZhipuClaudeFiles`：所有模型槽位（含 opus/sonnet/haiku/fable 别名与 subagent）钉 `ZHIPU_CC_SWITCH_MODEL`，并声明 `CLAUDE_CODE_MAX_CONTEXT_TOKENS = ZHIPU_CONTEXT_WINDOW_TOKENS`（1M；否则 Claude Code 按 200k 默认提前压缩）。CCS deeplink 的 zhipu 分支通过 `config=<base64 JSON>` 带同一份 env（`buildZhipuClaudeImportEnv`），模版 `zhipuModels['glm-5.3'].limit.context` 也读同一常量，三条导入路径同源。**回归易碎点（v0.2.0 rebase 实际踩到）**：`currentFiles` 的 `case 'zhipu'` 排在 `default:` 之前，而上游 v0.2.0 把所有非 OpenAI 组的 Codex tab 都放在 `default:` 里走 routed 模型目录——fork case 必须显式 `if codex → generateRoutedCodexFiles(apiBase, apiKey, 'zhipu')`，否则上游用例 "offers Codex catalog configuration for the zhipu routed group" 立刻红。另：不再写 `CLAUDE_CODE_ATTRIBUTION_HEADER=0`（见收编清单）。
...
| OpenAI ops 观测 + transport-error failover（`0d1d47f17`）| **本行有三层补丁，只有第 1 层受编译器保护，第 2/3 层 rebase 时必须手工核对。**<br>**① 签名扩展（编译期可见）**：`backend/internal/service/openai_upstream_transport_error.go` 的 `handleOpenAIUpstreamTransportError` 增加第 6 参 `upstreamURL`，request_error ops 事件标注上游端点；全部调用点（`openai_*`、`grok_audio.go`、`grok_media.go`、`openai_ws_http_bridge.go`）同步传 `safeUpstreamURL(<req>.URL.String())`。上游新增调用点必编译失败，按报错逐个补即可。<br>**② 行为替换（⚠ 不产生编译错误、不产生冲突，历史上已漏声明至少一轮）**：`openai_embeddings.go:99`、`openai_images.go:632`、`openai_images_responses.go:1727` 三处，上游原本是「内联 `appendOpsUpstreamError` + 写 502/返回 `fmt.Errorf`」的死路，fork 整段（各 10+ 行）替换为 `return nil, s.handleOpenAIUpstreamTransportError(...)`，从而获得 failover 换号 + 持久故障临时摘除。上游若在这三个函数里重写错误分支，rebase 会静默合成"上游内联版"而不报任何错——**每轮 rebase 必须逐个 `git diff <base>..HEAD -- <这三个文件>` 确认 hunk 还在**。<br>**③ passthrough 标志修正（⚠ 单个 bool 字面量，不产生冲突）**：v0.1.178 新增的三条 anthropic-native 调用点里，`openai_gateway_chat_completions_anthropic_native.go` 与 `openai_gateway_responses_anthropic_native.go` 是**协议转换**路径，上游误传 `passthrough=true`，fork 改 `false`；`messages_anthropic_native` 为零转换直通，`true` 正确保留。自 2026-08-20 起由 `backend/internal/service/openai_fork_source_contract_test.go` 的 `TestForkAnthropicNativePassthroughOpsTags` 源码级钉死（同文件的 `TestForkTransportErrorCallSitesTagUpstreamURL` 兜底第 ① 层）。<br>**豁免（A5，2026-08-20 判定）**：`openai_gateway_count_tokens.go` 的 `ForwardResponsesInputTokens`（v0.1.179 净新增，服务 `POST /v1/responses/input_tokens`）与同文件既有的 sibling **有意不接入** helper，保持内联 `setOpsUpstreamError` + 直写 502。理由：这是 preflight 估算路径而非转发路径——handler 侧压根没有 failover 循环（`openai_gateway_count_tokens.go` 只 log 返回值），helper 又刻意不写响应；且该路径自带本地估算降级（`writeOpenAIResponsesInputTokensFallback`），若接入 helper 就会让一次 token 预估探测触发 `tempUnscheduleOpenAITransportError`、把健康账号从**真实流量**调度里摘掉，风险面反而放大。`0d1d47f17` 当年也未转换那个 sibling，本次维持同一口径。已知代价：这两处不产生带 `UpstreamURL`/`Passthrough`/`Kind` 的 ops request_error 事件（`setOpsUpstreamError(c, 0, …)` 仍在，状态码与消息不丢）。若日后想补 ops 对齐，应抽一个「只记事件、不 failover、不摘除」的轻量函数，不要直接复用本 helper。～2026-08-20 v0.1.179：上游未新增 transport-error 调用点（adaptive 协议路由复用既有转发路径），rebase 后零编译失败，23 个非测试调用点全部保持第 6 参。<br>**～2026-08-25 v0.1.182：第 ① 层两次触发，其中一次 `go build` 抓不到。**（i）上游把 `openai_ws_http_bridge.go` 的上游请求构造挪进新的 rejected-field 重试循环，`upstreamReq` 不再是函数级变量，循环外两个调用点编译失败——改为在循环内留存 `upstreamRequestURL` 再喂给 `safeUpstreamURL`。（ii）上游新增 `backend/internal/service/plugin_security_regression_test.go`，内含 3 个直接调用 helper 的**测试**调用点；`go build ./...` 不编译测试文件，只有 `go vet -tags=unit ./...` / `go test -tags=unit` 才暴露。**结论：本层的"新增调用点必编译失败"保护网只覆盖非测试文件，每轮 rebase 必须同时跑 `go vet -tags=unit ./...`**（`openai_fork_source_contract_test.go` 的扫描也刻意跳过 `_test.go`，不兜这一类）。<br>**～2026-08-26 v0.1.183：本层零触发**，上游 182→183 未新增/移动任何调用点，23 个非测试调用点全部保持第 6 参，② 层三处 hunk 原样在位。<br>**⚠ 下轮重点观察（收编前兆）**：upstream/main 上 v0.1.183 **tag 之后**有 `4795650d2 fix(openai): record actual upstream endpoint on errors`（PR #6229），方向与本行第 ① 层高度重合——它给 `openai_gateway_service.go`/`handler/endpoint.go` 加"记录实际上游端点"的设施，并接进 `openai_gateway_cc_pipeline.go`、`openai_gateway_chat_completions.go`、`openai_gateway_forward.go`、`openai_gateway_messages.go`，而这四个文件正是 fork `safeUpstreamURL` 调用点所在。本轮基线取 tag 本身，**未收编**。下轮 rebase 时必须先判断：上游设施是否已覆盖 fork 的 `UpstreamURL` 标注语义——若是，第 ① 层按「谁覆盖面广用谁」转上游形态并让本行退场；若只是部分重合，注意别让两套标注互相覆盖（同一 ops 事件被写两次上游端点）。<br>**～2026-09-05 v0.2.0：#6229 已判定——不重合、不收编。** 上游那套是 `OpsErrorLogEntry.UpstreamEndpoint`（归一化路径如 `/v1/responses`，经 `SetActualOpenAIUpstreamEndpoint` 上下文键写入 handler 侧错误日志条目，用于分组统计），fork 第 ① 层填的是 `OpsUpstreamErrorEvent.UpstreamURL`（scheme+host+path，落在每次 attempt 的 request_error 事件上；上游自己对 HTTP 状态错误也填这个字段，fork 只是把 transport 错误补齐到同一字段）——两者是不同结构上的不同字段，互补而非双写，第 ① 层原样保留。本轮第 ① 层零触发：上游 183→v0.2.0 未新增/移动任何调用点（非测试 23 处、测试 15 处不变，`go vet -tags=unit` 全绿）；上游把 helper 内部类型/函数改名（`classifyOpenAITransportError`→`classifyUpstreamTransportError` 等），与 fork hunk 不相交、自动合并。② 层三处 hunk 原样在位（各 16 行 diff）。③ 层 false/false/true 由契约测试钉住。<br>**⚠ 下轮重点观察：post-tag PR #6179（`432ebc498`，upstream error proxy attribution）**直接命中本行三层：(i) 它在 helper 的 `appendOpsUpstreamError` 事件里加 `ProxyID`/`ProxyName`——fork 走 helper 的 23 个调用点自动继承，无需动作；(ii) 它同时给 ② 层三个文件（`openai_embeddings.go` / `openai_images.go` / `openai_images_responses.go`）里**上游那段内联 `appendOpsUpstreamError` 块**加了同样两个字段——那段正是 fork 整段替换成 helper 调用的死路，下轮 rebase 会在这三处冲突或静默合成「内联版 + 新字段」，正确解法是**保留 fork 的 helper 调用**（helper 已带 proxy 字段，覆盖面更广）；(iii) 它新增 `openai_upstream_transport_error_handle_test.go` 测试调用点，属「测试文件第 ① 层保护网盲区」，只有 `go vet -tags=unit` 能抓。 | 补丁 |
...
| CN 账号连接测试修补 | `backend/internal/service/account_test_service.go`。**～2026-08-25 v0.1.182：分发兜底整体退场，只剩一处 body 归一 + 一处防御纵深。** 上游 `01a008394`（PR #5913）补上 `responses` 分发、`a749673de` 补上 `anthropic` 分发，加上 v0.1.179 已有的 `adaptive`/`chat_completions`，`TestAccountConnection` 顶部的 CN switch 现在对 `GetAPIProtocol()` 的四个取值**完全穷尽**（该 getter 对非 CN 账号恒返回 `chat_completions`），fork 原先那个第二 switch 连同 `default:` arm 变成不可达死代码，已删除。<br>**剩余 fork 差异只有两条**：<br>① `case APIProtocolResponses:` 改投 fork 自有的 `backend/internal/service/account_test_service_cn_responses.go`（`testCNProviderResponsesConnection`）。上游把该分支投给通用 `testOpenAIAccountConnection`，URL/请求头/TLS profile/429 归位现在都已对齐（`f75c4161f` 让 URL 变平台感知），**唯一**剩余理由是请求体同构：上游探测不过 `normalizeDeepSeekResponsesRequestBody`，body 里没有 `store` 字段，而真实转发（`openai_gateway_forward.go:1284`、`openai_gateway_passthrough.go:606`）对 DeepSeek 恒发 `store=false`；且上游探测留了 `ShouldUseResponsesAPI` 逃生口会改打 `/v1/chat/completions`，而真实转发对固定 `responses` 协议明确拒绝被探针旧值覆盖。退场条件：上游给 `testOpenAIAccountConnection` 接上 body 归一并去掉逃生口。<br>② Claude 探测里 `GetAnthropicProtocolBaseURL` 非空时**无条件覆盖** `GetBaseURL()` —— 现已不可达（anthropic/adaptive 的 CN 账号都被上面的 switch 拦走，另一调用方 `routeAntigravityTest` 非 CN 平台该 getter 返回空串），**保留为防御纵深**：一旦上游收窄某个 arm，回落到这里的 CN 账号会带着供应商密钥去打 api.anthropic.com。<br>**已被上游收编（不再是 fork 补丁）**：`GetOpenAIProtocolAPIKey`（v0.1.182 基线自带，fork 仅剩的注释差已取上游形态）；anthropic 分发整体（上游 `testCNProviderAnthropicConnection` 覆盖面更广：`cnAnthropicBaseURLMisconfigHint`、无 api.anthropic.com 兜底、401 与 403 都落 `SetError`）。**注意 `?beta=true`**：fork 回归测试原先断言 CN anthropic 探测带 `?beta=true`，那是旧的通用 Claude 探测遗留产物，上游在 `account_test_service_cn_adaptive.go` 注释里明确列为缺陷，真实转发 `nativeAnthropicTargetURL`（"第三方端点保持朴素路径"）同样不附加——断言已改为朴素路径，勿再改回。回归测试：`backend/internal/service/account_test_service_cn_fork_dispatch_test.go`。～2026-09-05 v0.2.0：上游探测仍未接 body 归一（`account_test_service.go` 本轮只加了 `modelMetadataRegistry` 三个字段，`ShouldUseResponsesAPI` 逃生口仍在），① ② 两条差异原样保留，零冲突 | 补丁 + 新增 |
| OpenAI 模版默认型号 | 2026-09-05 起 OpenAI 相关使用模版统一钉 `gpt-6-astra`（上游默认 `gpt-5.5`）：`frontend/src/components/keys/UseKeyModal.vue`（`generateOpenAIFiles` / `generateOpenAIWsFiles` 的 `selectCodexCatalogModel` 首选、`generateRoutedCodexFiles` 的 openai/composite 首选、`generateOpenCodeConfig` 的 `openai/gpt-6-astra` pin 与 `openaiModels` 目录里新增的 `gpt-6-astra` 条目——带 `reasoning: true`，因为 models.dev 与 AI SDK 的硬编码 reasoning 名单都还不认识它，不显式声明 OpenCode 会拒绝这个 pin）、`frontend/src/utils/ccswitchImport.ts`（`OPENAI_CC_SWITCH_CODEX_MODEL`）、`frontend/src/composables/useModelWhitelist.ts`（`openaiModels` 首项 + 预设行，使模版推荐型号可白名单化）、`template/client-templates.json` 与三份 example（codex/websocket 段 `model`/`review_model` 同钉，`openai_opencode` 段 pin + `models` 块，`ccs_import` 示例 `model`）；对应断言在 `UseKeyModal.spec.ts` / `ccswitchImport.spec.ts` / `useModelWhitelist.spec.ts`。**后端注册**：同日 cherry-pick 上游两个未合并 PR（见下一行）后，计费兜底、alias 归一、Codex 映射表、`/v1/models` 默认列表都已认识 `gpt-6-astra`。**fork 独有语义差**：内置 OpenCode 目录里 `gpt-6` / `gpt-6-astra` 两个条目带 `reasoning: true`（上游 PR 没有）——models.dev 与 AI SDK 的硬编码 reasoning 名单都还不认识 GPT-6，OpenCode 只对显式声明的模型注入 forceReasoning（与 grok_opencode 同一机制，opencode#20815），`UseKeyModal.spec.ts` 的两个 `toEqual` 期望值也随之带 `reasoning: true`。上游把默认型号切到同一型号、且 AI SDK 收录 GPT-6 后本行退场 | 补丁 |
| 上游 PR 预收编（cherry-pick，待上游合并后退场） | 2026-09-05 以 `git cherry-pick -x` 从 upstream/main（`b1748c4ea`）之上取了两个**尚未合并**的 PR：<br>① **#6572** `3c8be0013` feat(openai): add GPT-6 Astra support（17 文件：`pkg/openai/constants.go` DefaultModels 加 `gpt-6-astra`/`gpt-6`、`billing_service.go` 与 `pricing_service.go` 官方价（$10/$50，缓存写 1.25×，>272K 长上下文 2×/1.5×，Fast 2×）、`openai_model_alias.go` 的 `isOpenAIGPT6AstraModel` + `normalizeKnownOpenAICodexModel`、`openai_codex_transform.go` 映射、`openai_codex_models_service.go` 的 1.05M 上下文/priority tier/max 档/vision、`openai_gateway_request_body.go` 的 max 档、前端 `useModelWhitelist.ts` 与 `UseKeyModal.vue` OpenCode 目录）。**本地 cherry-pick 与原 PR 不同**：前端 4 文件与 fork 的 gpt-6-astra 模版 pin 冲突，按上游形态解决并叠加 `reasoning: true`（见上一行），所以 patch-id 已变，**下轮 rebase 不会被自动去重**——若届时上游已合并，解法是整体取上游、只重放 `reasoning: true` 这两处；若上游改了定价/归一再合并，一律以上游为准。<br>② **#6620** `4aaf5cb73` fix(openai): enable Astra messages prompt caching（3 文件：`openai_compat_prompt_cache_key.go` 的 `shouldAutoInjectPromptCacheKeyForCompat` 放行 `gpt-6`/`gpt-6-astra` + 两个测试）。零冲突、原样落地，上游合并后 rebase 会按 patch-id 自动跳过。<br>两者都依赖 v0.2.0 已有的 `canonicalizeOpenAIModelAliasSpelling`，不依赖 tag 之后的任何提交。上游合并（或关闭）后本行退场 | 补丁 + 新增 |
| OpenAI `/v1/models` 默认清单精简 | `backend/internal/pkg/openai/constants.go` 的 `DefaultModels`：2026-09-05 按运营口径删掉 `gpt-5.4`、`gpt-5.2`、`gpt-image-1`、`gpt-image-1.5`，保留 gpt-5.6-sol / gpt-6-astra / gpt-6 / gpt-5.6 / gpt-5.6-terra / gpt-5.6-luna / gpt-5.5 / gpt-5.4-mini / gpt-5.3-codex-spark / codex-auto-review / gpt-image-2（11 个）。**首项必须仍是 `gpt-5.6-sol`**——上游 `constants_test.go` 的 `TestDefaultModelsPreferConcreteGPT56SolForAccountTests` 与 `account_handler_available_models_test.go` 都钉 `DefaultModels[0]` 为账号连通性测试默认。这份清单同时是：无账号映射时 `/v1/models` 的兜底输出、分组「自定义 /v1/models 模型列表」的候选来源（`admin_group.go` `defaultModelsListCandidateIDs`）、管理端账号可用模型列表、API key 账号合成 Codex 清单时的显示名来源。`DefaultTestModel = "gpt-5.4"` 常量未动（仍可经 alias 归一路由）。**连带测试差异**：`handler/gateway_models_test.go` 的 `TestGatewayModels_CustomModelsListFiltersDefaultFallbackModels` 与 `..._OpenAICustomModelsListKeepsOpenAIResponseShapeForDefaultFallback` 两个上游用例的夹具把 `gpt-5.4` 改为 `gpt-5.4-mini`（用例意图不变：只保留选中且存在于默认清单的条目），rebase 时上游若改这两个用例以上游为准再改回 mini。上游若自行收窄清单则退场 | 补丁 |
| Codex 合成清单优先级 | `backend/internal/service/openai_codex_models_service.go`：fork 表 `openaiCodexCatalogPriorities` + `openaiCodexCatalogPriority()`，`newConfiguredCodexModelDescriptor` 对已知 OpenAI slug 按官方目录 picker 优先级赋 `priority`（gpt-6-astra 1、gpt-5.6-sol 6、terra 7、luna 8、gpt-5.5 12、gpt-5.4 16、gpt-5.4-mini 23、5.3-codex-spark 26、gpt-5.2 29、codex-auto-review 43；别名 gpt-6=2、gpt-5.6=9），未知 alias 仍 50。**根因（2026-09-05 线上复现）**：组内账号一旦有模型映射，上游就用本地合成清单（全部 priority 50、按字母序）而非透传 ChatGPT 目录；Codex 客户端在 API key 鉴权下把网关清单按 slug **覆盖**内置表后按 priority 取第一个可见项当 Default——内置表里组不提供的 gpt-5.2（29）就成了 Default，钉住的 gpt-6-astra 掉到最后（用 codex-cli 0.153.4 内置表 + 线上组 2 清单模拟证实）。前端连带：`frontend/src/utils/codexCatalogConfig.ts` 新增 `isCodexCatalogModelHidden`，`UseKeyModal.vue` 的 `codexCatalogModelSlugs` 与 `api/codex.ts` 的 modelCount 跳过 `visibility: hide` 条目（官方目录里有 gpt-reserve 这类隐藏项）。同文件 `appendHiddenOfficialCodexModels`：OpenAI 组本地合成清单里只要出现过任一官方 slug，就把 `openaiCodexCatalogPriorities` 里其余**组不提供**的官方型号（排除 `codex-auto-*` 与媒体模型）追加为 `visibility: hide` 条目（追加在可见项之后），按 slug 覆盖客户端内置表里的残留可见项——网关「删掉」一个型号只是不再提及它，客户端内置的 gpt-5.2 只有被同名 hide 条目覆盖才会从选择器消失。Ark/GLM-only 的 OpenAI 平台组不含官方 slug，清单不变。**上游测试差异**：`TestBuildGroupConfiguredCodexModelsManifestExpandsSelectedModelCoveredByWildcardMapping` 的精确 slug 断言改用 fork helper `codexManifestVisibleModelSlugs`（只比可见项）。可上报上游。上游给合成条目补官方优先级并遮蔽未提供型号后退场。测试：`TestNewConfiguredCodexModelDescriptorMirrorsOfficialCatalogPriority`、`TestBuildGroupConfiguredCodexModelsManifestShadowsUnservedOfficialModels` | 补丁 |
| GLM 计费兜底补型号 | `backend/internal/service/billing_service.go`、`billing_service_test.go`：`initFallbackPricing` 加 `glm-5.3` 条目（与 5.2/5.1 同价）+ `getFallbackPricing` 的 `glm-5.3` 分支（必须排在裸 `glm-5` 之前，否则掉进 glm-5 档少收约 29%）。上游 v0.2.0 仍只到 glm-5.2；上游补上 glm-5.3 后本补丁退场。2026-09-02 引入（`2b3207dd2`），2026-09-05 补登记 | 补丁 |
| Grok ZDR 文件降级 | `backend/internal/service/openai_gateway_grok_model_input.go`、`_test.go`：`sanitizeGrokMessageContent` 对 `input_file`/`file` content part 不再原样转发（xAI 对 ZDR 账号整个关掉文件通道，必得 400 "File content is currently unsupported for ZDR customers"），改为 `grokUnsupportedFilePlaceholder` 一行英文备注让模型知道附件没送到；图片走 `input_image` 不受影响。上游 v0.2.0 在同文件加了 tool output 图片提升（PR #6325），hunk 不相交、自动合并。上游自行处理 ZDR 文件通道后退场。2026-09-02 引入（`f25e7d346`），2026-09-05 补登记 | 补丁 + 新增 |
...
> - Claude Code 模板的 `CLAUDE_CODE_ATTRIBUTION_HEADER=0` — 上游 `c03776604`（v0.2.0，"preserve Claude attribution headers"）从 Anthropic/Grok 全部模板与测试里移除了这一行；fork 的 GLM 模板（#11 `generatePinnedClaudeFiles`）与 CCS 导入 env（`buildZhipuClaudeImportEnv`）2026-09-05 rebase 时同步跟随，勿再加回。
