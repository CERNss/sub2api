...
Sidecar 集成也可以把已存在的 API Key 转移给正确的所有者，并可选更新分组绑定、额度或清零已用额度：

```bash
POST /api/v1/admin/api-keys/:id/transfer
```

转移响应会确认 `data.api_key.user_id`、`quota` 与 `quota_used`，转移成功后会使 API Key 鉴权缓存失效。

### 用户 Token API Key 自动化

外部用户侧工具可以以普通用户身份登录，换取 JWT token 对，然后调用与网页控制台相同的用户 API Key 接口。能力范围与该用户账号完全一致：分组绑定仅限该用户可用分组，Turnstile、backend-mode 登录限制、账号状态校验和 TOTP 2FA 均保持生效。
...
登录返回 `requires_2fa` 与 `temp_token` 时，用 `/token/2fa` 完成两步验证；用 `/token/refresh` 刷新 access/refresh token 对。拿到 access token 后以 `Authorization: Bearer <access_token>` 调用用户接口：
...
`PUT /api/v1/keys/:id/group` 只轮换 key 绑定的分组，不改变 key 值、额度、有效期、IP 名单、状态和速率限制；`group_id: 0` 表示解绑分组。生成的 API Key 仍是模型网关调用的凭证；登录 JWT 只用于用户 REST API 自动化与密钥管理。

### 自定义菜单外部打开模式

管理员自定义菜单项现在可以在原有 iframe 模式之外选择外部打开模式。外部菜单会在新标签页打开配置的绝对 URL，并把当前 Sub2API JWT 以 `token` 查询参数透传，方便 sidecar 管理工具建立自己的会话。
